...
def run_network(inputs, viewdirs, frame_time, fn, embed_fn, embeddirs_fn, embedtime_fn, netchunk=1024*64,
                embd_time_discr=True, features=None, feat_dim_appearance=0):
...
def render_path(render_poses, render_times, hwf, chunk, render_kwargs, features=None, gt_imgs=None, savedir=None,
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        ...
        rgb, disp, acc, _ = render(H, W, focal, features=feature, chunk=chunk, c2w=c2w[:3,:4], frame_time=frame_time, **render_kwargs)
...
def create_nerf(args):
    ...
    network_query_fn = lambda inputs, viewdirs, ts, network_fn, features : run_network(inputs, viewdirs, ts, network_fn,
                                                                features=features,
                                                                embed_fn=embed_fn,
